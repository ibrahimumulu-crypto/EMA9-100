// TALGO 0012 - Toggle Butonlar + BOS Cizgi Duzeltme
//
...
string MAButonAdi  = PANEL_PREFIX + "MAButon";
string GHHButonAdi = PANEL_PREFIX + "GHHButon";

// TALGO 0012: Toggle durumlari
bool   MACizgiGorunur    = true;
bool   YapiGorselGorunur = true;
...
void YapiSwingOnayla(datetime zaman, double fiyat, int yon, int barIndex)
{
   ...
   // TALGO 0012: BOS cizgisi icin onceki HH/LL seviyelerini kaydet
   double eskiHHFiyat = SonHHFiyat;
   datetime eskiHHZamani = SonHHZamani;
   double eskiLLFiyat = SonLLFiyat;
   datetime eskiLLZamani = SonLLZamani;
   ...
   if(tip == YAPI_HH)
   {
      if(YapiTrendYonu == 1 && eskiHHFiyat > 0)
      {
         // Yukseliste yeni HH = BOS — onceki HH seviyesinden kesikli cizgi
         YapiBOSCizgiCiz(eskiHHZamani, zaman, eskiHHFiyat, clrLime);
         YapiBOSSayisi++;
   ...
   {
      if(YapiTrendYonu == -1 && eskiLLFiyat > 0)
      {
         // Dususte yeni LL = BOS — onceki LL seviyesinden kesikli cizgi
         YapiBOSCizgiCiz(eskiLLZamani, zaman, eskiLLFiyat, clrRed);
         YapiBOSSayisi++;
         ArrayResize(YapiBOSSeviyeleri, YapiBOSSayisi);
         ArrayResize(YapiBOSZamanlari, YapiBOSSayisi);
         ArrayResize(YapiBOSYonleri, YapiBOSSayisi);
         YapiBOSSeviyeleri[YapiBOSSayisi - 1] = eskiLLFiyat;
         YapiBOSZamanlari[YapiBOSSayisi - 1] = zaman;
         YapiBOSYonleri[YapiBOSSayisi - 1] = -1;
         // BOS etiketi — cizginin yaninda
         string bosAdi = YAPI_BOS_PREFIX + (string)zaman + "_BOS";
         ...
         {
            double bosluk = _Point * 10;
            ObjectCreate(0, bosAdi, OBJ_TEXT, 0, zaman, eskiLLFiyat - bosluk);
            ObjectSetString(0, bosAdi, OBJPROP_TEXT, "BOS");
...
//============================================================
// TALGO 0012: Belirli prefix ile baslayan tum nesnelerin
// gorunurlugunu degistirir (toggle icin).
//============================================================
void NesneleriGizleGoster(string prefix, bool gorunur)
{
   int zamandilimleri = gorunur ? OBJ_ALL_PERIODS : OBJ_NO_PERIODS;
   int toplamNesne = ObjectsTotal(0);
   for(int i = toplamNesne - 1; i >= 0; i--)
   {
      string nesneAdi = ObjectName(0, i);
      if(StringFind(nesneAdi, prefix) == 0)
         ObjectSetInteger(0, nesneAdi, OBJPROP_TIMEFRAMES, zamandilimleri);
   }
}
...
void PanelOlustur()
{
   ...
   // TALGO 0012: MA toggle butonu - Apply'in saginda
   ObjectCreate(0, MAButonAdi, OBJ_BUTTON, 0, 0, 0);
   ObjectSetInteger(0, MAButonAdi, OBJPROP_CORNER, CORNER_RIGHT_UPPER);
   ObjectSetInteger(0, MAButonAdi, OBJPROP_XDISTANCE, 45);
   ObjectSetInteger(0, MAButonAdi, OBJPROP_YDISTANCE, 25);
   ObjectSetInteger(0, MAButonAdi, OBJPROP_XSIZE, 35);
   ObjectSetInteger(0, MAButonAdi, OBJPROP_YSIZE, 20);
   ObjectSetString(0, MAButonAdi, OBJPROP_TEXT, "MA");
   ObjectSetInteger(0, MAButonAdi, OBJPROP_FONTSIZE, 9);
   ObjectSetInteger(0, MAButonAdi, OBJPROP_COLOR, clrBlack);
   ObjectSetInteger(0, MAButonAdi, OBJPROP_BGCOLOR, clrSilver);
   ObjectSetInteger(0, MAButonAdi, OBJPROP_STATE, false);
   ObjectSetInteger(0, MAButonAdi, OBJPROP_SELECTABLE, false);
   ObjectSetInteger(0, MAButonAdi, OBJPROP_HIDDEN, true);

   // TALGO 0012: GHH toggle butonu - ikinci satir
   ObjectCreate(0, GHHButonAdi, OBJ_BUTTON, 0, 0, 0);
   ObjectSetInteger(0, GHHButonAdi, OBJPROP_CORNER, CORNER_RIGHT_UPPER);
   ObjectSetInteger(0, GHHButonAdi, OBJPROP_XDISTANCE, 83);
   ObjectSetInteger(0, GHHButonAdi, OBJPROP_YDISTANCE, 48);
   ObjectSetInteger(0, GHHButonAdi, OBJPROP_XSIZE, 40);
   ObjectSetInteger(0, GHHButonAdi, OBJPROP_YSIZE, 20);
   ObjectSetString(0, GHHButonAdi, OBJPROP_TEXT, "GHH");
   ObjectSetInteger(0, GHHButonAdi, OBJPROP_FONTSIZE, 9);
   ObjectSetInteger(0, GHHButonAdi, OBJPROP_COLOR, clrBlack);
   ObjectSetInteger(0, GHHButonAdi, OBJPROP_BGCOLOR, clrSilver);
   ObjectSetInteger(0, GHHButonAdi, OBJPROP_STATE, false);
   ObjectSetInteger(0, GHHButonAdi, OBJPROP_SELECTABLE, false);
   ObjectSetInteger(0, GHHButonAdi, OBJPROP_HIDDEN, true);
...
void OnDeinit(const int reason)
{
   ...
   ObjectDelete(0, MAButonAdi);
   ObjectDelete(0, GHHButonAdi);
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   ...
   if(sparam == ButonAdi)
   {
      ...
      if(MetniPeriyodaCevir(girilenMetin, yeniPeriyot))
      {
         ...
         if(!MACizgiGorunur)
            NesneleriGizleGoster(MA2_CIZGI_PREFIX, false);
      }
      else
      {
   ...
   else if(sparam == MAButonAdi)
   {
      ObjectSetInteger(0, MAButonAdi, OBJPROP_STATE, false);
      MACizgiGorunur = !MACizgiGorunur;
      NesneleriGizleGoster(MA1_CIZGI_PREFIX, MACizgiGorunur);
      NesneleriGizleGoster(MA2_CIZGI_PREFIX, MACizgiGorunur);
      ObjectSetInteger(0, MAButonAdi, OBJPROP_BGCOLOR, MACizgiGorunur ? clrSilver : clrDimGray);
   }
   else if(sparam == GHHButonAdi)
   {
      ObjectSetInteger(0, GHHButonAdi, OBJPROP_STATE, false);
      YapiGorselGorunur = !YapiGorselGorunur;
      NesneleriGizleGoster(YAPI_ETIKET_PREFIX, YapiGorselGorunur);
      NesneleriGizleGoster(YAPI_BOS_PREFIX, YapiGorselGorunur);
      NesneleriGizleGoster(YAPI_MSB_PREFIX, YapiGorselGorunur);
      NesneleriGizleGoster(YAPI_ZIGZAG_PREFIX, YapiGorselGorunur);
      ObjectSetInteger(0, GHHButonAdi, OBJPROP_BGCOLOR, YapiGorselGorunur ? clrSilver : clrDimGray);
   }
...
void OnTick()
{
   ...
   // TALGO 0012: Toggle kapaliysa yeni segmentleri de gizle
   if(!MACizgiGorunur)
   {
      NesneleriGizleGoster(MA1_CIZGI_PREFIX, false);
      NesneleriGizleGoster(MA2_CIZGI_PREFIX, false);
   }
   ...
   // TALGO 0012: Toggle kapaliysa yeni yapi nesnelerini de gizle
   if(!YapiGorselGorunur)
   {
      NesneleriGizleGoster(YAPI_ETIKET_PREFIX, false);
      NesneleriGizleGoster(YAPI_BOS_PREFIX, false);
      NesneleriGizleGoster(YAPI_MSB_PREFIX, false);
      NesneleriGizleGoster(YAPI_ZIGZAG_PREFIX, false);
   }
